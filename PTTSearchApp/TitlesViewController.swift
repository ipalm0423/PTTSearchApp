...
class TitlesViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        self.searchTitle(self.hint, startRow: self.titles.count + 1, counts: 10)
...
//internet
    func searchTitle(hint: String, startRow: Int, counts: Int) {
        self.showIndicator = true
        self.indicatorText = "搜尋中"
        var url = Singleton.sharedInstance.serverURL + "search/searchArticle"
        println("search for title...")
        //搜尋帳號
        Alamofire.request(.GET, url, parameters: ["agent" : "iphone", "hint" : hint, "startCount" : startRow, "counts" : counts]).responseObject { (response: mapTitles?, error: NSError?) -> Void in
